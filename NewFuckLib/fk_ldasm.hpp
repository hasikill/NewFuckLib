...
namespace fk
{
    ...
    public:
        ...
        unsigned int __fastcall dasm(void* code, ldasm_data* ld, uint32_t is64 = false)
            /*
             Description:
        ...
        // Get function size
        unsigned long __fastcall size_of_proc(void* proc)
        {
            uint32_t  Length;
            uint8_t* pOpcode;
            uint32_t  Result = 0;
            ...
            do
            {
                Length = dasm((uint8_t*)proc, &data, is_x64);
        ...
        void* __fastcall resolve_jmp(void* Proc)
        {
            uint32_t  Length;
            uint8_t* pOpcode;
            ldasm_data data = { 0 };
            ...
            if (Length == 5 && data.opcd_size == 1 && *pOpcode == 0xE9)
            {
                ...
                return resolve_jmp((void*)((uintptr_t)Proc + delta + Length));
            }
    ...
    public:
        ...
        static unsigned int instr_size(void* code, uint32_t is64 = false)
        {
            ldasm_data ld;
            return obj().dasm(code, &ld, is64);
        }
    };
}
